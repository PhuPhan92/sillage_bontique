...
//        resp.sendRedirect("/translate");
    ...
    private void editCustomer(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        isValidPhone(req, customer, errors);
...
//            part.write();
    ...
    private void isValidAddress(HttpServletRequest req, Customer customer, List<String> errors) {
        ...
        if (!ValidateUtils.isAddressValid(address) || address.isEmpty()) {
            errors.add("Địa chỉ không hợp lệ hoặc bỏ trống. Chỉ chứa từ từ 5-15 kí và bắt đầu A-Za-z0-9");
        }
    ...
    private void isValidName(HttpServletRequest req, Customer customer, List<String> errors) {
        ...
        if (!ValidateUtils.isNameValid(name) || name.isEmpty()) {
            errors.add("Tên không hợp lệ hoặc bỏ trống. Chỉ chứa từ từ 5-10, không có ký tự đặt biệt và bắt đầu bằng một chử cái in hoa");
        }
        customer.setName(name);
    }
    private void isValidPhone(HttpServletRequest req, Customer customer, List<String> errors) {
        String phone = req.getParameter("phone");
        if (!ValidateUtils.isPhoneValid(phone) || phone.isEmpty()) {
            errors.add("Số điện thoại không hợp lệ hoặc bỏ trống. Phải bắt đầu bằng số 0 và không có chử cái hay ký tự đặt biệt");
        }
