...
import com.codegym.casemd3.service.IOrderService;
...
public class MSOrderService extends DBContext implements IOrderService {
    ...
    String ADD_ORDER = "INSERT INTO `order` (`address`, `phone`, `name`, `total`) VALUES (?, ?, ?, ?)";
    @Override
...
//        VALUES ('28 NTP', '3456789', 'Quang', '12');\n";
        try {
            ...
            for (int i = 0; i < order.getOrderItems().size(); i++) {
                ...
                connection.prepareStatement("INSERT INTO `sillage_bontique`.`order_item` (`id_order`, `id_product`, `quantity`) VALUES ('21', '3', '1')").executeUpdate();
...
//         Thực thi gọi procedure không có tham số
    ...
    public void saveOrderBySPWithParameter(Order order) {
        ...
        System.out.println(jsonInString);
        try {
            CallableStatement callableStatement = connection.prepareCall("call sillage_bontique.spSaveOrderWithParameter(?, ?)");
            callableStatement.setObject(1, jsonInString);
