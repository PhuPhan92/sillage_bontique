...
public class MSCustomerService extends DBContext implements ICustomerService {
    ...
    private static final String INSERT_CUSTOMER = "INSERT INTO `customer` ( `name`, `createdat`, `address`, `image`, `idcustomertype`, `phone`) VALUES (?, ?,?, ?, ?,? )";
    ...
    private static final String EDIT_CUSTOMER = "UPDATE `customer` SET `name` = ?, `createdat` = ?, `address` = ?, `image` =?, `idcustomertype` = ? ,`phone` = ? WHERE (`id` = ?)";
    private static final String CHECK_IMAGE_EXIST = "SELECT * FROM `customer` where image = ?;";
...
//            PreparedStatement preparedStatement = connection.prepareStatement(SELLECT_ALL_CUSTOMER);
    ...
    private Customer getCustomerFromRs(ResultSet rs) throws SQLException {
        ...
        Customer customer = new Customer(id, name, uCreatedAt, address, image,phone);
